...
@router.get(
    "/{grinder_id}/profiles/suggest",
    response_model=Optional[schemas.GrinderProfileOut],
)
def suggest_profile(
    grinder_id: int,
    ...
    """
    Return the best-matching grinder profile for the given bean + method
    combination. Returns null if no suitable profile exists.
    Ranked: exact > bean-only > method-only > general.
    """
    grinder = crud.get_grinder(db, grinder_id)
    if not grinder:
        raise HTTPException(status_code=404, detail="Grinder not found")
    return crud.suggest_grinder_profile(db, grinder_id, bean_id, method_id)


@router.post(
    "/{grinder_id}/profiles",
    response_model=schemas.GrinderProfileOut,
    status_code=status.HTTP_201_CREATED,
)
def create_profile(
    grinder_id: int,
    data: schemas.GrinderProfileCreate,
    db: Session = Depends(get_db),
    _: models.User = Depends(get_current_user),
):
    grinder = crud.get_grinder(db, grinder_id)
...
@router.delete(
    "/{grinder_id}/profiles/{profile_id}",
